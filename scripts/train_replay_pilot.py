"""Small, isolated GRPO/replay experiment. Existing experiment code is unchanged.
...
def probe_scores(policy, rows, probe, layer, end_id, micro):
    ...
    with torch.no_grad():
        for start in range(0, len(rows), micro):
            values.extend(replay_logits(policy, rows[start:start + micro], probe, layer,
                                        "mean_cot", end_id).sigmoid().cpu().tolist())
...
def train(cfg):
    ...
    with (run_dir / "metrics.jsonl").open("x") as metric_file, (run_dir / "rollouts.jsonl").open("x") as rollout_file:
        for step in range(1, cfg["steps"] + 1):
            ...
            scores = probe_scores(policy, rows, probe, cfg["layer"], end_id, micro)
            rewards = task_reward - lam * scores
            ...
            metric_file.flush()
            for row, score, monitor_score, advantage, reward in zip(rows, scored, scores, advantages, task_reward):
                rollout_file.write(json.dumps({**row, **dataclasses.asdict(score), "step": step,
                                              "probe_score": float(monitor_score), "advantage": float(advantage),
