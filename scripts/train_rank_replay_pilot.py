"""Post-hoc rank-penalty follow-up. The locked four-arm runner is unchanged.
...
def train(cfg):
    if cfg.get("penalty_transform") != "within_prompt_average_logit_rank":
        raise ValueError("This follow-up requires its explicitly declared rank-penalty configuration")
    ...
    with (run_dir / "metrics.jsonl").open("x") as metric_file, (run_dir / "rollouts.jsonl").open("x") as rollout_file:
        for step in range(1, cfg["steps"] + 1):
            ...
            monitor_penalties = group_rank_penalties(monitor_logits, group_size)
            ...
            record = {"step": step, "n": len(rows), "lambda": lam, "replay_weight": mu,
                      ...
                      "within_group_rank_variance": float(monitor_penalties.reshape(-1, group_size).var(axis=1).mean()),
