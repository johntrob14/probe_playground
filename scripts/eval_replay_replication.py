"""Token-preserving compact evaluation for the rank-replication experiment.
...
def main():
    ...
    config.update(task_ids=task_ids, audit_task_ids=sorted({r["task_id"] for r in audit}),
                  ...
                  exact_token_ids_retained=True, source_evaluator="scripts/eval_replay_checkpoint.py",
    ...
    for filename, records in (("scored.jsonl", rows), ("audit_scored.jsonl", audit)):
        with (args.out / filename).open("x") as f:
            for row in records:
                # Retain exact IDs for future fixed-text comparisons; no activation arrays are stored.
                compact = dict(row)
